...
const normalizeDesignation = (value: string) => value.trim().toUpperCase();
const isStrictlyPositiveThreeDecimals = (value: string) => {
  const trimmed = value.trim();
  if (!/^\d+(?:[.,]\d{1,3})?$/.test(trimmed)) return false;
...
export const ProductsPage = () => {
  ...
  const parseImportFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    raw.forEach((row, index) => {
      ...
      const rawPrice = String(row.purchase_unit_price_ht ?? '').trim();
      if (!isStrictlyPositiveThreeDecimals(rawPrice)) pushRowError(line, 'purchase_unit_price_ht invalide (nombre > 0 avec max 3 décimales).');
      ...
      if (!lineErrors.length) {
        parsedRows.push({ row_number: line, designation: normalizeDesignation(designation), nature: nature as ProductNature, pct_code: pctCode, barcode, purchase_unit_price_ht: Number(rawPrice.replace(',', '.')), vat_rate_id: vatRate!.id, vat_rate_label: vatRate!.label, laboratory_id: laboratory!.id, laboratory_designation: laboratory!.designation });
      } else {
  ...
  const revalidateAnomalyRow = (row: ImportAnomalyRow) => {
    ...
    const price = Number(row.purchase_unit_price_ht.replace(',', '.'));
    ...
    if (!isStrictlyPositiveThreeDecimals(row.purchase_unit_price_ht)) errs.push('purchase_unit_price_ht invalide (> 0, max 3 décimales)');
    ...
    const valid: ImportRow = { row_number: row.row_number, designation: normalizeDesignation(row.designation), nature: nature as ProductNature, pct_code: pctCode, barcode, purchase_unit_price_ht: price, vat_rate_id: vatRate!.id, vat_rate_label: vatRate!.label, laboratory_id: laboratory!.id, laboratory_designation: laboratory!.designation };
  ...
  const setAnomalyField = (rowNumber: number, field: EditableAnomalyField, value: string) => {
    const normalized = field === 'designation' ? value.toUpperCase() : value;
    setAnomalyRows((current) => current.map((r) => r.row_number === rowNumber ? { ...r, [field]: normalized } : r));
  };
