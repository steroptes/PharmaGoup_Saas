...
export const LaboratoriesPage = () => {
  ...
  return (<div className="grid"><Card><h1>Laboratoires</h1><p>Créer et gérer les fiches laboratoires, puis piloter le catalogue hiérarchique.</p>{feedback && <p style={{ marginTop: 12 }}>{feedback}</p>}</Card>
    ...
    {isCatalogModalOpen && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'grid', placeItems: 'center', zIndex: 45 }}><Card style={{ width: 'min(980px, 92vw)', maxHeight: '88vh', overflow: 'auto', padding: 12 }}>
      ...
      {filtered && <>
        <div style={{ marginTop: 10, padding: 10, borderRadius: 8, background: filtered.business_units.length === 0 ? '#fff7ed' : '#ecfeff', border: '1px solid #e5e7eb' }}><strong>ℹ️ {filtered.business_units.length === 0 ? 'Mode sans BU' : 'Mode avec BU'}</strong><p style={{ marginTop: 4 }}>{filtered.business_units.length === 0 ? 'Le catalogue est géré à la racine (produits et brands).' : 'Le catalogue est géré par BU, puis brands et produits.'}</p></div>
        <Card style={{ marginTop: 10, padding: 10 }}>
          <details open>
            <summary style={{ cursor: 'pointer', fontWeight: 600 }}>Catalogue (Racine)</summary>
            <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <Button onClick={() => { setCatalogView({ type: 'root', label: 'Racine du laboratoire' }); setCatalogActionModal('create_root_product'); }}>+ Produit racine</Button>
              <Button variant="secondary" onClick={() => { setCatalogView({ type: 'root', label: 'Racine du laboratoire' }); setCatalogActionModal('create_root_brand'); }}>+ Brand racine</Button>
              <Button variant="ghost" onClick={() => setCatalogActionModal('create_bu')}>+ BU</Button>
            </div>
            {filtered.root_products.length > 0 && <div style={{ overflow: 'auto', marginTop: 8 }}><Table><TableHead><TableRow><TableHeaderCell>Produit racine</TableHeaderCell><TableHeaderCell>Nature</TableHeaderCell></TableRow></TableHead><TableBody>{filtered.root_products.map((product) => <TableRow key={product.id}><TableCell>{product.designation}</TableCell><TableCell>{product.nature}</TableCell></TableRow>)}</TableBody></Table></div>}
            {filtered.root_group_brands.map((brand) => <details key={brand.id} style={{ marginTop: 8 }}><summary style={{ cursor: 'pointer' }}>Brand: {brand.name} ({brand.products.length})</summary><div style={{ marginTop: 8, display: 'flex', gap: 8 }}><Button variant="secondary" onClick={() => { setCatalogView({ type: 'group_brand', id: brand.id, label: brand.name }); setCatalogActionModal('create_root_product'); }}>+ Produit dans brand</Button></div>{brand.products.length > 0 && <div style={{ overflow: 'auto', marginTop: 8 }}><Table><TableHead><TableRow><TableHeaderCell>Produit</TableHeaderCell><TableHeaderCell>Nature</TableHeaderCell></TableRow></TableHead><TableBody>{brand.products.map((product) => <TableRow key={product.id}><TableCell>{product.designation}</TableCell><TableCell>{product.nature}</TableCell></TableRow>)}</TableBody></Table></div>}</details>)}
            {filtered.business_units.map((bu) => <details key={bu.id} style={{ marginTop: 8 }}><summary style={{ cursor: 'pointer', fontWeight: 600 }}>BU: {bu.name} (P:{bu.products.length} / B:{bu.group_brands.length})</summary><div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button onClick={() => { setCatalogView({ type: 'business_unit', id: bu.id, label: bu.name }); setCatalogActionModal('create_root_product'); }}>+ Produit BU</Button><Button variant="secondary" onClick={() => { setCatalogView({ type: 'business_unit', id: bu.id, label: bu.name }); setCatalogActionModal('create_root_brand'); }}>+ Brand BU</Button></div>{bu.products.length > 0 && <div style={{ overflow: 'auto', marginTop: 8 }}><Table><TableHead><TableRow><TableHeaderCell>Produit BU</TableHeaderCell><TableHeaderCell>Nature</TableHeaderCell></TableRow></TableHead><TableBody>{bu.products.map((product) => <TableRow key={product.id}><TableCell>{product.designation}</TableCell><TableCell>{product.nature}</TableCell></TableRow>)}</TableBody></Table></div>}{bu.group_brands.map((brand) => <details key={brand.id} style={{ marginTop: 8 }}><summary style={{ cursor: 'pointer' }}>Brand: {brand.name} ({brand.products.length})</summary><div style={{ marginTop: 8 }}><Button variant="secondary" onClick={() => { setCatalogView({ type: 'group_brand', id: brand.id, label: brand.name }); setCatalogActionModal('create_root_product'); }}>+ Produit brand</Button></div>{brand.products.length > 0 && <div style={{ overflow: 'auto', marginTop: 8 }}><Table><TableHead><TableRow><TableHeaderCell>Produit Brand</TableHeaderCell><TableHeaderCell>Nature</TableHeaderCell></TableRow></TableHead><TableBody>{brand.products.map((product) => <TableRow key={product.id}><TableCell>{product.designation}</TableCell><TableCell>{product.nature}</TableCell></TableRow>)}</TableBody></Table></div>}</details>)}</details>)}
          </details>
        </Card>
      </>}
